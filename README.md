# LiltingChannelLabo

[lilting channel](https://www.lilting.ch/) の実験・ベンチマーク記事のうち、ソースを見た方が伝わるものの置き場。  
Companion code for experiments and benchmarks published on [lilting channel](https://www.lilting.ch/) ([EN](https://www.lilting.ch/en/)).

記事の公開日ごとに `YYYY/MM/DD/<記事slug>/` でディレクトリを切る。各ディレクトリは公開時点のスナップショットで、原則として更新しない。

## Index

| 日付 | 実験 | 記事 |
|---|---|---|
| 2026-09-23 | [CoreS3からTailscale・東京DERP経由でWAVを取得](2026/09/23/m5stack-cores3-tailscale-direct-voice-server/) | [JA](https://lilting.ch/articles/m5stack-cores3-tailscale-direct-voice-server) / [EN](https://lilting.ch/en/articles/m5stack-cores3-tailscale-direct-voice-server) |
| 2026-09-20 | [OEM版ACR1251Uのファームウェアを書き換えて非接触側を動かす](2026/09/20/acr1251u-ntag-ez-sign-windows/) | [JA](https://www.lilting.ch/articles/acr1251u-ntag-ez-sign-windows) |
| 2026-08-22 | [Z3で18bitうるう年判定の定数を合成](2026/08/22/leap-year-magic-number-z3/) | [JA](https://www.lilting.ch/articles/leap-year-magic-number-z3) / [EN](https://www.lilting.ch/en/articles/leap-year-magic-number-z3) |
| 2026-08-20 | [メルセンヌ数曜日計算 vs 素朴な%7ベンチ](2026/08/20/fast-day-of-week-mersenne-benchmark/) | [JA](https://www.lilting.ch/articles/fast-day-of-week-mersenne-benchmark) |
| 2026-08-06 | [Next.js 16.3 / Astro 7 / Nuxt 4.5 同一デザイン実測比較](2026/08/06/nextjs-16-3-astro-nuxt-benchmark/) | [JA](https://www.lilting.ch/articles/nextjs-16-3-astro-nuxt-benchmark) / [EN](https://www.lilting.ch/en/articles/nextjs-16-3-astro-nuxt-benchmark) |
| 2026-08-06 | [QwenのMoEルーター観測と観測ベースキャッシュの対照実験](2026/08/06/qwen-moe-router-cache-experiment/) | [JA](https://www.lilting.ch/articles/qwen-moe-router-cache-experiment) |
| 2026-09-22 | [Qwen-Image 2.1 × TaylorSeerキャッシュ（M1 Max）](2026/09/22/qwen-image-2-1-taylorseer-cache-m1-max/) | [JA](https://www.lilting.ch/articles/qwen-image-2-1-taylorseer-cache-m1-max) / [EN](https://www.lilting.ch/en/articles/qwen-image-2-1-taylorseer-cache-m1-max) |
| 2026-09-22 | [Qwen-Image 2.1の部分編集と位置ずれの測定](2026/09/22/qwen-image-2-1-edit-pixel-perfect-output-resolution/) | [JA](https://www.lilting.ch/articles/qwen-image-2-1-edit-pixel-perfect-output-resolution) / [EN](https://www.lilting.ch/en/articles/qwen-image-2-1-edit-pixel-perfect-output-resolution) |
